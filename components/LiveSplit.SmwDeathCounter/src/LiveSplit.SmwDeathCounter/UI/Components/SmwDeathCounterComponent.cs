...
using LiveSplit.SmwDeathCounter.Snes;

namespace LiveSplit.UI.Components;

public class SmwDeathCounterComponent : IComponent
{
    // SNES address $7E:0071 — Mario player animation. Value 0x09 = "dying".
    // See kaizosplits Watchers.cs: DiedNow => ShiftTo(playerAnimation, 9).
    ...
    public SmwDeathCounterComponentSettings Settings { get; }
    ...
    public SmwDeathCounterComponent(LiveSplitState state)
    {
        this.state = state;

        bool allowGamepads = state.Settings.HotkeyProfiles.First().Value.AllowGamepadsAsHotkeys;
        Settings = new SmwDeathCounterComponentSettings(allowGamepads);
    ...
    {
        if (e == Settings.ResetKey) { Deaths = 0; }
    }
    ...
    private void Poll()
    {
        ...
        if (!emu.ReadWramByte(PlayerAnimationOffset, out byte anim))
        {
            hasPrevious = false;
            return;
        }
    ...
    public void Update(IInvalidator invalidator, LiveSplitState state, float width, float height, LayoutMode mode)
    {
        ...
        nameLabel.Text = string.IsNullOrEmpty(Settings.Label) ? "Deaths" : Settings.Label;
        valueLabel.Text = Deaths.ToString() + StatusGlyph();
    ...
    private string StatusGlyph() => emu.IsAttached ? "" : " ◌";
